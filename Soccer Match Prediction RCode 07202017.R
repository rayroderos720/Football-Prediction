...
# AwayTeam1 <- "Hull"
  ...
  Match.opposite <- paste(AwayTeam1,HomeTeam1)
  ...
  z.match.opposite <- subset(prod.data,
                    prod.data$Match==Match.opposite &
                      which.max(prod.data$Key))
  z.match.opposite <- z.match.opposite[order(z.match.opposite$Date,decreasing=TRUE),]
  z.opposite <- z.match.opposite$FTR[1]
  z.opposite <- chartr("ADH", "HDA", z.opposite)
  ...
  pred.tab <- cbind(HomeTeam1,AwayTeam1, 
                    as.data.frame(test.z$predict),predict2 = as.data.frame(test.z3$predict), 
                    actual = z.result,
                    previous = z.previous,
                    opposite = z.opposite)
  colnames(pred.tab) <- c("Home","Away","predict1","predict2","actual","previous","opposite")
  pred.tab
  return(pred.tab)  
  
}

predict.outcome("Aberdeen","Hamilton")
predict.outcome("Motherwell","Rangers")
predict.outcome("Chelsea","Arsenal")
predict.outcome("Chelsea","Hull")
predict.outcome("Man United","Liverpool")
predict.outcome("Swansea","Man United")
predict.outcome("Southampton","Swansea")
predict.outcome("Arsenal","Leicester")



unique(english.1617.pl$HomeTeam)

##-----------------------------------------------------------------------------##


environment.name <- paste('Prediction',format(Sys.time(), "%Y-%m-%d %I-%p"), "RData", sep = ".")
save.image(file=environment.name)
